...
def _month_bounds(year, month):
    start = timezone.make_aware(datetime(year, month, 1, 0, 0))
    if month == 12:
        nxt = datetime(year + 1, 1, 1)
    else:
        nxt = datetime(year, month + 1, 1)
    return start, timezone.make_aware(nxt)


def _monthly_summary_data(user, year, month):
    """Aggregated month stats shared by the CSV and PDF reports."""
    start, end = _month_bounds(year, month)
    entries = FoodEntry.objects.filter(user=user, recorded_at__gte=start, recorded_at__lt=end)
    logged_days = {}
    for e in entries:
        day = timezone.localtime(e.recorded_at).date()
        logged_days.setdefault(day, []).append(e)
    days_logged = len(logged_days)
    kcal_values = [float(e.calories or 0) for e in entries]
    protein_values = [float(e.protein or 0) for e in entries]
    avg = lambda vals: round(sum(vals) / len(vals), 1) if vals else 0.0
    sets = WorkoutSet.objects.filter(
        session__user=user, session__started_at__gte=start,
        session__started_at__lt=end, completed=True,
    ).exclude(set_type="warmup")
    volume_by_muscle = {}
    for s in sets.select_related("exercise"):
        for muscle in s.exercise.primary_muscles or []:
            volume_by_muscle[muscle] = volume_by_muscle.get(muscle, 0) + 1
    sleep = SleepEntry.objects.filter(user=user, date__gte=start.date(), date__lt=end.date())
    # New PRs this month: exercises whose best-ever e1RM was set this month.
    from .services import personal_records
    month_prs = [r for r in personal_records(user) if r["date"].year == year and r["date"].month == month]
    return {
        "year": year, "month": month,
        "days_logged": days_logged,
        "avg_kcal": avg(kcal_values), "avg_protein": avg(protein_values),
        "total_entries": entries.count(),
        "workout_sets": sets.count(),
        "volume_by_muscle": dict(sorted(volume_by_muscle.items(), key=lambda kv: -kv[1])),
        "avg_sleep": round(sum(float(x.hours) for x in sleep) / len(sleep), 1) if sleep else 0.0,
        "prs": month_prs,
        "logged_days": sorted(logged_days.keys()),
    }
...
def monthly_report(request, year, month):
    ...
    data = _monthly_summary_data(request.user, year, month)
    ...
    if fmt == "csv":
        ...
        w.writerow(["Nutrition", "Days logged", data["days_logged"]])
        w.writerow(["Nutrition", "Average calories", data["avg_kcal"]])
        w.writerow(["Nutrition", "Average protein (g)", data["avg_protein"]])
        w.writerow(["Nutrition", "Food entries", data["total_entries"]])
        w.writerow(["Training", "Completed working sets", data["workout_sets"]])
        for muscle, count in data["volume_by_muscle"].items():
            w.writerow(["Training", f"Sets · {muscle}", count])
        w.writerow(["Recovery", "Average sleep (h)", data["avg_sleep"]])
        for r in data["prs"]:
            w.writerow(["Personal records", r["exercise"].name,
                        f"{r['weight']:g} kg × {r['reps']} (e1RM {r['e1rm']:g} kg, {r['date']:%d %b %Y})"])
        response = HttpResponse(buf.getvalue(), content_type="text/csv; charset=utf-8")
        response["Content-Disposition"] = f'attachment; filename="fitness-report-{stamp}.csv"'
        response["Cache-Control"] = "no-store"
        return response
    # PDF via Pillow (already a dependency): draw the report page as an
    # image, save multi-page as PDF. No new heavy dependencies.
    from PIL import Image as _Img, ImageDraw as _Draw, ImageFont as _Font
    from django.conf import settings as _settings
    W, H = 1240, 1754  # A4 @150dpi
    ink, muted, green, bg = (233, 240, 230), (157, 178, 160), (109, 141, 84), (24, 33, 27)
    img = _Img.new("RGB", (W, H), bg)
    d = _Draw.Draw(img)
    try:
        font_h = _Font.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 64)
        font_s = _Font.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 40)
        font_b = _Font.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 34)
        font_m = _Font.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 28)
    except OSError:
        font_h = font_s = font_b = font_m = _Font.load_default()
    y = 90
    d.text((90, y), "Form · Monthly report", font=font_h, fill=green); y += 110
    d.text((90, y), f"{datetime(year, month, 1):%B %Y}", font=font_s, fill=ink); y += 90
    def section(title, rows):
        nonlocal y
        d.text((90, y), title.upper(), font=font_s, fill=green)
        y += 58
        for label, value in rows:
            d.text((120, y), label, font=font_b, fill=muted)
            d.text((W - 90 - d.textlength(str(value), font=font_b), y), str(value), font=font_b, fill=ink)
            y += 52
        y += 30
    section("Nutrition", [
        ("Days logged", data["days_logged"]),
        ("Average calories", f"{data['avg_kcal']:.0f} kcal"),
        ("Average protein", f"{data['avg_protein']:.0f} g"),
        ("Food entries", data["total_entries"]),
    ])
    training_rows = [("Completed working sets", data["workout_sets"])]
    training_rows += [(f"Sets · {m}", c) for m, c in list(data["volume_by_muscle"].items())[:8]]
    section("Training", training_rows)
    section("Recovery", [("Average sleep", f"{data['avg_sleep']:.1f} h")])
    if data["prs"]:
        d.text((90, y), "NEW PERSONAL RECORDS", font=font_s, fill=green); y += 58
        for r in data["prs"]:
            d.text((120, y), f"{r['exercise'].name}", font=font_b, fill=ink)
            val = f"{r['weight']:g} kg × {r['reps']} · e1RM {r['e1rm']:g} kg · {r['date']:%d %b}"
            d.text((W - 90 - d.textlength(val, font=font_m), y + 4), val, font=font_m, fill=muted)
            y += 50
    from io import BytesIO as _BytesIO
    buf = _BytesIO()
    img.save(buf, format="PDF")
    response = HttpResponse(buf.getvalue(), content_type="application/pdf")
    response["Content-Disposition"] = f'attachment; filename="fitness-report-{stamp}.pdf"'
    response["Cache-Control"] = "no-store"
    return response
